...
async function loadAllPrompts() {
  console.log('すべてのプロンプトを読み込んでいます...');
  const searchResultsContainer = document.getElementById('search-results');

  // searchResultsContainer が存在しない場合はエラーを出力して終了
  if (!searchResultsContainer) {
    console.error('search-results 要素が見つかりませんでした。');
    return;
  }

  try {
    // prompts.json を取得
    const response = await fetch('./prompts.json'); // パスを相対パスに変更
    if (!response.ok) {
      throw new Error(`HTTPエラー: ${response.status}`);
    }
    const prompts = await response.json();
    ...
    // すべてのプロンプトを filteredPrompts に格納
    filteredPrompts = prompts;

    // 初期描画時にデフォルトのソートを適用
    sortResults(currentSort);
  } catch (error) {
    console.error('プロンプトの読み込み中にエラーが発生しました:', error);
    searchResultsContainer.innerHTML = '<p>プロンプトを読み込めませんでした。</p>';
  }
}
