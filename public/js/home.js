document.addEventListener('DOMContentLoaded', () => {
  ...
  function createPickupCard(article) {
    ...
    link.href = `/article`; // 適宜 article.id などを使って動的にする
    link.className = 'pickup-card-link'; // スタイル調整用
  
    link.innerHTML = `
      <div class="pickup-card">
        <img src="${article.image}" alt="${article.category}" class="pickup-image">
        <div class="pickup-meta">
          <span class="pickup-date">${article.created_at}</span> 
  ...
  function createPromptCard(article) {
    ...
    card.innerHTML = `
      <div class="prompt-meta">
        <span class="prompt-date">${article.created_at}</span> 
